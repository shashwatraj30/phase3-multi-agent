...
MAX_CHARS = 2000  # truncation limit per input block
...
def analyze_paper(state: dict) -> dict:
    ...
    for pdf_path in state["pdf_paths"]:
        try:
            ...
            problem = query_paper(index, "core problem research question motivation")[:MAX_CHARS]
            novelty = query_paper(index, "novelty contribution proposed method solution")[:MAX_CHARS]
...
def connect_papers(state: dict) -> dict:
    try:
        analyses = "\n\n".join([
            ...
            for i, a in enumerate(state["analyses"])
        ])
        gaps = "\n\n".join([g[:MAX_CHARS] for g in state["gaps"]])
        response = llm.invoke([HumanMessage(content=(
...
def synthesize_gaps(state: dict) -> dict:
    try:
        connections = state["connections"][:MAX_CHARS]
        response = llm.invoke([HumanMessage(content=(
            ...
            "If no novel solution is derivable for a gap, explicitly say so.\n\n"
